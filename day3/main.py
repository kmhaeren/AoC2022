...
with open("day3/input.txt") as fp:
    lines = [line.strip() for line in fp.readlines()]
    compartements = [ [lines[i], lines[i+1],lines[i+2]] for i in range(0, len(lines), 3)]


print(sum([ score(set(l1).intersection(set(l2)).intersection(set(l3)).pop()) for (l1, l2, l3) in compartements ]))